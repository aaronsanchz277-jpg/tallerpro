...
async function modalFichaRecepcion(repId) {
  const { data: rep } = await sb.from('reparaciones')
    .select('*, vehiculos(patente,marca,modelo,color), clientes(nombre,telefono)')
    .eq('id', repId).single();
  if (!rep) return;
  ...
  openModal(`
    ...
    <button class="btn-primary" style="background:var(--success)" onclick="confirmarIngresoConSafeCall('${repId}')">✓ CONFIRMAR INGRESO AL TALLER</button>
    <button class="btn-secondary" onclick="closeModal()">Cancelar</button>`);
}

async function confirmarIngresoConSafeCall(repId) {
  await safeCall(async () => {
    await confirmarIngreso(repId);
  }, null, 'No se pudo confirmar el ingreso');
}
...
async function confirmarIngreso(repId) {
  ...
  const ficha = {
    kilometraje: document.getElementById('fr-km').value,
    combustible: document.getElementById('fr-comb').value,
    fecha_entrega_aprox: document.getElementById('fr-entrega').value,
    accesorios,
    estado_interior: estadoInterior,
    danos: document.getElementById('fr-danos').value,
    observaciones: document.getElementById('fr-obs').value,
    fecha_ingreso: new Date().toISOString()
  };
  await sb.from('reparaciones').update({ ficha_recepcion: ficha, estado: 'en_progreso' }).eq('id', repId);
  clearCache('reparaciones');
  toast('✓ Vehículo ingresado al taller', 'success');
  closeModal();
...
async function generarCartaConformidad(repId) {
  ...
  openModal(`
    ...
    <div style="margin-top:1rem;display:flex;gap:.5rem">
      <button class="btn-primary" style="flex:1;margin:0" onclick="window.print()">🖨️ Imprimir</button>
      <button class="btn-primary" style="flex:1;margin:0;background:var(--success)" onclick="marcarEntregadoConSafeCall('${repId}')">📨 Marcar Entregado</button>
      <button class="btn-secondary" style="flex:1;margin:0" onclick="closeModal()">Cerrar</button>
    </div>`);
}

async function marcarEntregadoConSafeCall(repId) {
  await safeCall(async () => {
    await marcarEntregado(repId);
  }, null, 'No se pudo marcar como entregado');
}
