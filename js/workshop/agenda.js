...
async function detalleCita(id) {
  ...
  document.getElementById('main-content').innerHTML = `
    ...
    <div style="display:flex;gap:.5rem;flex-wrap:wrap">
      ...
      ${c.estado==='confirmada'?`<button onclick="crearRepDesdeCitaConSafeCall('${c.cliente_id||''}','${c.vehiculo_id||''}','${hjs(c.descripcion||'')}')" style="flex:1;background:rgba(0,229,255,.12);color:var(--accent);border:1px solid rgba(0,229,255,.3);border-radius:10px;padding:.6rem;font-family:var(--font-head);font-size:.82rem;cursor:pointer">🔧 Crear reparación</button>`:''}
