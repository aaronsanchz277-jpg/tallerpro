// ─── REPORTE COMPARATIVO (Este mes vs. Mes anterior, Este año vs. Año anterior) ─
async function reporteComparativas() {
  ...
  const [
    { data: repsActual }, { data: repsAnterior },
    { data: ventasActual }, { data: ventasAnterior },
    { data: gastosActual }, { data: gastosAnterior },
    { data: repsAnioActual }, { data: repsAnioAnterior },
    { data: ventasAnioActual }, { data: ventasAnioAnterior },
    { data: gastosAnioActual }, { data: gastosAnioAnterior }
  ] = await Promise.all([
    sb.from('reparaciones').select('costo, costo_repuestos').eq('taller_id', tid()).eq('estado', 'finalizado').gte('fecha', inicioMesActual).lte('fecha', finMesActual),
    sb.from('reparaciones').select('costo, costo_repuestos').eq('taller_id', tid()).eq('estado', 'finalizado').gte('fecha', inicioMesAnterior).lte('fecha', finMesAnterior),
    sb.from('ventas').select('total').eq('taller_id', tid()).eq('estado', 'completado').gte('created_at', inicioMesActual).lte('created_at', finMesActual + 'T23:59:59'),
    sb.from('ventas').select('total').eq('taller_id', tid()).eq('estado', 'completado').gte('created_at', inicioMesAnterior).lte('created_at', finMesAnterior + 'T23:59:59'),
    sb.from('gastos_taller').select('monto').eq('taller_id', tid()).gte('fecha', inicioMesActual).lte('fecha', finMesActual),
    sb.from('gastos_taller').select('monto').eq('taller_id', tid()).gte('fecha', inicioMesAnterior).lte('fecha', finMesAnterior),
    sb.from('reparaciones').select('costo, costo_repuestos').eq('taller_id', tid()).eq('estado', 'finalizado').gte('fecha', inicioAnioActual).lte('fecha', finMesActual),
    sb.from('reparaciones').select('costo, costo_repuestos').eq('taller_id', tid()).eq('estado', 'finalizado').gte('fecha', inicioAnioAnterior).lte('fecha', finAnioAnterior),
    sb.from('ventas').select('total').eq('taller_id', tid()).eq('estado', 'completado').gte('created_at', inicioAnioActual).lte('created_at', finMesActual + 'T23:59:59'),
    sb.from('ventas').select('total').eq('taller_id', tid()).eq('estado', 'completado').gte('created_at', inicioAnioAnterior).lte('created_at', finAnioAnterior + 'T23:59:59'),
    sb.from('gastos_taller').select('monto').eq('taller_id', tid()).gte('fecha', inicioAnioActual).lte('fecha', finMesActual),
    sb.from('gastos_taller').select('monto').eq('taller_id', tid()).gte('fecha', inicioAnioAnterior).lte('fecha', finAnioAnterior)
  ]);

  function calcIngresos(reps, ventas) {
    const ingReps = (reps || []).reduce((s, r) => s + parseFloat(r.costo || 0), 0);
      ...
      <div id="comparativas-content">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem">
          <div style="background:var(--surface2);border-radius:10px;padding:.6rem;text-align:center">
            ...
            <div style="font-family:var(--font-head);font-size:1.1rem">${formatearMoneda(ingresosMesActual / ((repsActual?.length || 0) + (ventasActual?.length || 0) || 1))}</div>
          </div>
          <div style="background:var(--surface2);border-radius:10px;padding:.6rem;text-align:center">
