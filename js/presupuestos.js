...
// Aprobar presupuesto: crea reparación y copia ítems
async function aprobarPresupuesto(id) {
  confirmar(t('confirmarAprobar'), async () => {
    ...
    // Insertar ítems detallados en reparacion_items
    const items = p.items || [];
    if (items.length > 0) {
...
async function rechazarPresupuesto(id) {
  confirmar(t('confirmarRechazar'), async () => {
...
async function eliminarPresupuesto(id) {
  confirmar(t('confirmarEliminar'), async () => {
...
// Modal nuevo/editar presupuesto (unificado)
