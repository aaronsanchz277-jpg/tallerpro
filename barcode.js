...
async function ia_ejecutar(a) {
  // Sanitizar: la IA a veces manda "null", "undefined", "ultimo" como strings
  ...
  const cleanInt = (v) => { const n = parseInt(v); return isNaN(n) ? null : n; };
  ...
  switch(a.accion) {
    ...
    case 'crear_vehiculo': {
      const { data: nuevo, error } = await sb.from('vehiculos').insert({ patente:(a.patente||'').toUpperCase(), marca:a.marca||'', modelo:a.modelo||'', anio:cleanInt(a.anio), cliente_id:cleanUUID(a.cliente_id), taller_id:tid() }).select().single();
      if (error) { ia_addMsg('Error: '+error.message, false); return; }
    ...
    case 'crear_quickservice': {
      const items = a.items || [];
      const total = items.reduce((s,i)=>s+parseFloat(i.precio||0)*(i.cantidad||1),0);
      const { error } = await sb.from('quickservices').insert({
